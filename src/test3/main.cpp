...
void setup() {
  ...
  if (version != 0x12) Serial.println("Warning: unexpected version; continuing");
  enterLoRaStandby();
  setFrequency(868000000);
  Serial.println("Frequency set to 868000000 Hz (EU).");
  uint8_t pa = readRegister(REG_PA_CONFIG);
  printHexDec("PaConfig before TX:", pa);
  Serial.println("Ready to send test packet every 6s. Watch for TxDone.");
}
...
void loop() {
  ...
  Serial.print("DIO0 pin state: ");
  Serial.println(digitalRead(PIN_DIO0));
}
